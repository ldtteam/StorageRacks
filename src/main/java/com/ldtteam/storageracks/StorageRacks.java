...
public class StorageRacks
{
    ...
    public static void on(final PlayerInteractEvent.LeftClickBlock event)
    {
        ...
        {
            if (event.getWorld().getBlockState(event.getPos()).getBlock() instanceof RackBlock)
            {
                ((RackBlock) event.getWorld().getBlockState(event.getPos()).getBlock()).checkUpgrade(event.getPos(), event.getPlayer());
            }
        }
    }

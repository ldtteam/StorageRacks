...
public class RecipeProvider implements DataProvider
{
    ...
    public CompletableFuture<?> run(@NotNull final CachedOutput cache)
    {
        for (final DeferredBlock<RackBlock> state : ModBlocks.racks)
        {
            ...
            keys.put("F", new RecipeIngredientKeyJson(new RecipeIngredientJson(BuiltInRegistries.ITEM.getKey((state.get().getWoodType().getMaterial().asItem())).toString(), false)));
            keys.put("C", new RecipeIngredientKeyJson(new RecipeIngredientJson(BuiltInRegistries.ITEM.getKey(state.get().frameType.getCreationCost()).toString(), false)));
            keys.put("X", new RecipeIngredientKeyJson(new RecipeIngredientJson(Tags.Items.RODS_WOODEN.location().toString(), true)));

            final ShapedRecipeJson json = new ShapedRecipeJson("racks", pattern, keys, new RecipeResultJson(1, BuiltInRegistries.ITEM.getKey(state.get().asItem()).toString()));
            this.models.add(new Tuple<>(json, BuiltInRegistries.BLOCK.getKey(state.get()).getPath()));
        }
        ...
        for (final DeferredBlock<CornerBlock> state : ModBlocks.corners)
        {
            ...
            final ShapedRecipeJson json = new ShapedRecipeJson("corners", pattern, keys, new RecipeResultJson(8, BuiltInRegistries.ITEM.getKey(state.get().asItem()).toString()));
            this.models.add(new Tuple<>(json, BuiltInRegistries.BLOCK.getKey(state.get()).getPath()));
        }
    ...
    private void generateControllerRecipe(final CachedOutput cache, final ControllerBlock state, final Item prev)
    {
        ...
        final Map<String, RecipeIngredientKeyJson> keys = new HashMap<>();
        keys.put("S", new RecipeIngredientKeyJson(new RecipeIngredientJson(BuiltInRegistries.ITEM.getKey(state.getBuildMaterial()).toString(), false)));
        keys.put("P", new RecipeIngredientKeyJson(new RecipeIngredientJson(BuiltInRegistries.ITEM.getKey(prev).toString(), false)));

        final ShapedRecipeJson json = new ShapedRecipeJson("controllers", pattern, keys, new RecipeResultJson(1, BuiltInRegistries.ITEM.getKey(state.asItem()).toString()));
        this.models.add(new Tuple<>(json, BuiltInRegistries.BLOCK.getKey(state).getPath()));
    }
